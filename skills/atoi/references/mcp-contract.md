# Atoi MCP v1 contract

This reference records the public companion contract implemented by the
authenticated Atoi MCP bridge.

## Transport and authentication

The client-facing transport is local stdio. Its server command is `atoi` with
arguments `mcp` and `serve`.

The installed Atoi CLI resolves the configured Convex site, appends `/mcp`,
...
Atoi's backend remains authoritative for every tool call. Review exact
